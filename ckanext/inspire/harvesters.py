...
class GDIHarvester(InspireHarvester,SingletonPlugin):
    '''
    A Harvester for CSW servers, for now focused on Geodatenkatalog-DE
    '''
    implements(IHarvester)
    ...
    def info(self):
        return {
            'name': 'gdi',
            'title': 'CSW Server like GDI Geodatenkatalog',
            'description': 'A server that implements OGC\'s Catalog Service for the Web (CSW) standard, espacially the the German Geodata Catalog'
            }

    def gather_stage(self,harvest_job):
        log.debug('In GDIHarvester gather_stage')
        # Get source URL
        url = harvest_job.source.url
